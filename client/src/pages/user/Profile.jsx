      ...
      <div className="w-full mr-4">
        ...
        <div className="grid grid-cols-4 gap-4 ">
          {ImageChoose.map((Data, index) => (
            <div
              key={Data.id}
              className={` transitiona-all duration-300 rounded-full cursor-pointer p-1 w-fit ${selectProfile === Data.id ? "border-2 border-green " : "border-none"}  `}
            >
              <img
                onClick={() => setSelectProfile(Data.id)}
                className={`${selectProfile === Data.id ? "max-h-[108px] w-[108px]" : "w-28 max-h-28 "} object-cover object-top rounded-full  `}
                src={Data.img}
                alt=""
                srcSet=""
              />
        ...
        <div>
          <div className="w-full flex justify-center ">
            {selectedImage ? (
              ...
              <p className="font-sfreg text-lg ">
                Select a picture to set as profile
              </p>
            )}
          </div>
          {selectedImage && (
            <div className=" flex justify-center w-full p-4 mt-4 space-x-4 ">
              <button className="bg-[#ffffff20] text-white px-12 py-3 cursor-pointer text-xl font-sfreg rounded-3xl  ">
                cancel
              </button>
              <button
                className="bg-green text-dark px-12 py-3 cursor-pointer text-xl font-sfreg rounded-3xl  "
                onClick={updateImage}
              >
                update
              </button>
