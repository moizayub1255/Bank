...
import { useAuth, useAcc } from "../../contexts";
const API_URL = import.meta.env.VITE_API_URL;

const SignIn = () => {
  const navigate = useNavigate();
  const { login: loginAuth } = useAuth();
  const { setAccount } = useAcc();
  ...
  const handleAccInfo = async (email) => {
    try {
      ...
      if (response.ok && data.success) {
        alert(
          `Account created successfully! Your Account Number: ${data.accountno}`,
        );
        // Update account context with token
