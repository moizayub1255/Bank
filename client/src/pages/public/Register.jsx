...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        // Expect backend to return token (user), and accountToken (account)
        localStorage.setItem("token", data.token);
        if (data.accountToken && data.data && data.data.accountno) {
          localStorage.setItem("accountToken", data.accountToken);
          alert(
            `Account created successfully! Your Account Number: ${data.data.accountno}`,
          );
        } else {
          alert(
            "User registered successfully! But account number could not be retrieved.",
          );
        }
        setFormData({
          name: "",
  ...
  // Removed handleAccInfo, now handled in handleSubmit
    ...
    <div>
      <div className="flex h-full justify-center items-center min-h-screen py-10">
        <div className="flex flex-col justify-center items-center space-y-4 p-10 px-16 bg-[#ffffff10] rounded-3xl">
            ...
            {["Male", "Female", "Other"].map((gender) => (
              <div key={gender}>
                ...
                <label
                  ...
                  className="bg-[#ffffff10] px-6 py-2 rounded-2xl border-[1px]  peer-checked:text-black peer-checked:bg-green cursor-pointer transition-all duration-300"
                >
                  {gender}
                </label>
              </div>
          ...
          <button
            className="bg-green text-black w-full py-2 rounded-xl"
            onClick={handleSubmit}
          >
            Submit
          </button>
          <Link to="/signin" className="text-end w-full">
