...
const Deposit = () => {
  ...
  const handleApprove = async (accountno, amount) => {
    if (
      !window.confirm(
        `Are you sure you want to approve the deposit for Account No: ${accountno}?`,
      )
    )
      return;
    ...
    const notificationData = {
      ...
      type: "deposit",
      message: `PKR ${amount} added to your account successfully!`,
    };
    ...
    try {
      const response = await axios.put(
        ...
        {
          balance: deposits.find((deposit) => deposit.accountno === accountno)
            ?.balance,
        },
      );
      ...
      setDeposits((prev) =>
        prev.filter((deposit) => deposit.accountno !== accountno),
      );
    } catch (err) {
  ...
  return (
    <div className="w-full flex flex-col items-center  ">
      <h1 className="text-3xl font-lato text-white mb-12 w-full ">
          ...
          deposits.map((deposit) => (
            <div className="bg-white text-black min-w-66 w-fit p-6 text-xl font-jet rounded-3xl">
              <div className="space-y-2" key={deposit._id}>
                ...
                <div className="text-5xl text-end font-bold">
                  PKR {deposit.balance}
                </div>
                <div className="flex w-full space-x-2 mt-4">
                  ...
                  <button
                    onClick={() =>
                      handleApprove(deposit.accountno, deposit.balance)
                    }
