    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const authdata = await axios.get(
          `${API_URL}/api/users/register/data/${response.data.email}`,
        );
        ...
        <div>
          ...
          <div className="bg-dark p-10 text-lg font-sfreg space-y-4 rounded-4xl h-fit">
            ...
            <div className="flex space-x-2 ">
              ...
              <div>{authData?.gender}</div>
            </div>
            <div className="flex space-x-2 ">
              <p className="text-light  ">Role: </p> <div>{authData?.role}</div>
            </div>
            <div className="flex space-x-2 ">
              <p className="text-light  ">Created At: </p>
              <div>{new Date(authData?.createdAt).toLocaleDateString()}</div>
            </div>
          </div>
          <Link
            to={`/employee/cibil-score/d/${accountno}`}
            onClick={handleSubmit}
          >
            <div className=" w-full text-center cursor-pointer font-sfpro  bg-green text-dark py-4 rounded-3xl text-xl mt-6 flex items-center justify-center space-x-4">
              {" "}
        ...
        <div>
          <div className="flex space-x-6 mb-6 ">
            <div className="bg-dark h-fit text-light p-10 text-lg font-sfreg space-y-4 rounded-4xl ">
              <div className="mr-20">Transactions Did: </div>
              <div className="text-end font-jet text-4xl text-green ">
          ...
          <div className="bg-dark p-10 text-lg font-sfreg space-y-4 rounded-4xl">
            <div className="flex space-x-2">
              <p className="text-light">Account Number:</p>{" "}
              <div>{loanData?.accountno}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Full Name:</p>{" "}
              <div>{loanData?.fullname}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Date of Birth:</p>{" "}
              <div>{loanData?.dob}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Gender:</p>{" "}
              <div>{loanData?.gender}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Contact:</p>{" "}
              <div>{loanData?.contact}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Email:</p> <div>{loanData?.email}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Employment Type:</p>{" "}
              <div>{loanData?.employmenttype}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Company Name:</p>{" "}
              <div>{loanData?.companyname}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Job Title:</p>{" "}
              <div>{loanData?.jobtitle}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Income:</p>{" "}
              <div>₹{loanData?.income}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Loan Amount:</p>{" "}
              <div>₹{loanData?.loanamount}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Loan Purpose:</p>{" "}
              <div>{loanData?.loanpurpose}</div>
            </div>
            <div className="flex space-x-2">
              <p className="text-light">Submitted At:</p>{" "}
              <div>{new Date(loanData?.submittedat).toLocaleDateString()}</div>
            </div>
          </div>
        </div>
      </div>
